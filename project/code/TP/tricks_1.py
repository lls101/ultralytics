...
def draw_box(img, box, color):da
    cv2.rectangle(img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), color, thickness=2)
    return img

if __name__ == '__main__':
    postfix = 'jpg'
    img_path = r'D:\Workspace\datasets\wgsid\wgsid0\test\images'
    label_path = r'D:\Workspace\datasets\wgsid\wgsid0\test\labels'
    predict_path = r'D:\Workspace\YOLO\models\ultralytics\runs\detect\predict7\labels'
    save_path = 'vis'
    classes = ['uva']
